...
def generate_cpp_source(binding: ASBindingFile, header_include: str) -> str:
    ...
    for class_function in binding.class_functions:
        ...
        return_type = _resolve_cpp_type(
            class_function.cpp_return_type,
            type_map,
        )
...
def generate_cpp_header(binding: ASBindingFile) -> str:
    ...
    type_map = {
        as_type.name: as_type.cpp_type
        for as_type in binding.types
    }

    gen.write(
        generator.generate_comment(
            "GENERATED BY THE CE IDL, DO NOT MODIFY"
        )
    )
    ...
    gen.write(
        f'#include "{generator.ANGELSCRIPT_CLASS_INCLUDE}"'
    )
    gen.write(
        f'#include "{generator.ANGELSCRIPT_MACRO_INCUDE}"'
    )
    ...
    gen.write(
        f"{CLASS_NAME}(Runtime& runtime, asIScriptEngine& script_engine)"
    )
    gen.write(
        "    : IScriptBinding(runtime, script_engine) {}"
    )
    gen.write("")
    ...
    if binding.class_functions:
        ...
        for class_function in binding.class_functions:
            sig = (
                class_function.cpp_signature
                or class_function.signature
            )

            params = _cpp_parameter_list(sig)

            function_name = (
                class_function.generated_name
                or class_function.name
            )
...
def main() -> int:
    try:
        ...
        print(f"[Info] Writing metadata: {metadata.resolve()}")
        metadata.write_text(
            json.dumps(
                {
                    "class": CLASS_NAME,
