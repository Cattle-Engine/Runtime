...
IDL_GENERATOR = ROOT / "tools" / "build" / "idl.py"
...
# Compatiblity crutch from when I was testing
def generated_path_unique_path(directory: Path, name: str, extension: str) -> Path:
...
def generate_binding_registry(build: Path) -> None:
    ...
    gen.write("#include <memory>")
    gen.write("#include <string_view>")
    ...
    gen.begin_struct("AutoGeneratedBindings")
...
def build_idl(build_dir: Path) -> None:
    ...
    for generated in generated_dir.glob("*.generated.*"):
        if generated not in expected_files:
            generated.unlink()
            log(f"Removed stale generated file {generated.name}")

    generate_binding_registry(build_dir)
