...
namespace CE::Assets::Model3DImporter {
    ...
    ) : mRenderer(renderer), mVFS(vfs), mGPUMeshManager(mesh_manager), mMaterialManager(mat_manager), mTextureManager(tex_man) {}
    ...
    SDL_Surface* ModelImporter::DecodeSurface(
        const aiScene* scene,
        const std::string& assimp_path,
        ...
        if (!assimp_path.empty() && assimp_path[0] == '*') {
            ...
            try {
                ...
                if (tex->mHeight == 0) {
                    result = DecodeCompressedTexture(
    ...
    Renderer::Resources::MaterialHandle ModelImporter::LoadAssimpMaterial(
        ...
        auto resolveHandle = [&](
            const std::string& assimp_path,
            SDL_Surface* surface
        ) -> Renderer::Resources::TextureHandle {
            if (assimp_path.empty()) return 0;
            ...
            auto handle = mTextureManager.CreateTextureFromData(
                ...
                Renderer::TextureFormat::RGBA8, surface->pitch,
        ...
        SDL_Surface* albedo_surf    = albedo_path.empty()    ? CreateWhiteSurface() : DecodeSurface(scene, albedo_path,    model_path, surfaceCache);
        SDL_Surface* normal_surf    = normal_path.empty()    ? CreateWhiteSurface() : DecodeSurface(scene, normal_path,    model_path, surfaceCache);
        SDL_Surface* roughness_surf = roughness_path.empty() ? CreateWhiteSurface() : DecodeSurface(scene, roughness_path, model_path, surfaceCache);
        SDL_Surface* metallic_surf  = metallic_path.empty()  ? CreateWhiteSurface() : DecodeSurface(scene, metallic_path,  model_path, surfaceCache);

        Renderer::Resources::TextureHandle albedo_handle = resolveHandle(albedo_path.empty()    ? "__white_albedo"    : albedo_path,    albedo_surf);
        Renderer::Resources::TextureHandle normal_handle = resolveHandle(normal_path.empty()    ? "__white_normal"    : normal_path,    normal_surf);
        ...
        if (!mr_found) {
            ...
            mr_handle = mTextureManager.CreateTextureFromData(
                mr_surf->w, mr_surf->h, mr_surf->pixels,
                Renderer::TextureFormat::RGBA8, mr_surf->pitch,
                Renderer::TextureFilter::Linear,
                Renderer::TextureWrap::Repeat,
                mr_key,
                batch
    ...
    Renderer::Resources::Model ModelImporter::ImportModel(std::string path) {
...

        {
            ...
            for (uint32_t i = 0; i < scene->mNumMaterials; ++i) {
                model.Materials.push_back(LoadAssimpMaterial(
                    scene, scene->mMaterials[i], model, path,
                    gpuHandleCache, surfaceCache, batch
                ));
            }
