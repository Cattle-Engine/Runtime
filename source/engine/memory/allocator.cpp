...
namespace {
    ...
    struct MallocAllocator {
        ...
        template <typename U>
        bool operator==(const MallocAllocator<U>&) const { return true; }
        template <typename U>
        bool operator!=(const MallocAllocator<U>&) const { return false; }
    };
    ...
    void TrackAllocation(std::size_t size) {
        ...
        while (alive > peakAlive && !stats.peakAliveAllocations.compare_exchange_weak(
            peakAlive, alive, std::memory_order_relaxed)) {
            }
        ...
        while (current > peak &&
            !stats.peakBytes.compare_exchange_weak(peak, current, std::memory_order_relaxed)) {
            }
    }
...
namespace CE::Memory {
    void EnableTracking(bool enabled) { trackingEnabled.store(enabled, std::memory_order_relaxed); }
    
    bool IsTrackingEnabled() { return trackingEnabled.load(std::memory_order_relaxed); }
