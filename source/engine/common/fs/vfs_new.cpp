...
namespace CE::Common::FS::VFS {
    ...
    std::string VFS::ResolveVirtualPath(const std::string& path) {
        ...
        for (const auto& part : p) {
            ...
            if (part == "..") {
                if (!parts.empty()) {
                    CE_LOG(LogLevel::Info,
                        "[VFS] Resolving '..', removing component '{}'",
                        parts.back()
                    );
    ...
    VFS::ResolvedPath VFS::ResolvePath(std::string_view path) {
        ...
        for (auto& mount : mMountPoints) {
            ...
            CE_LOG(LogLevel::Info,
                "[VFS] Checking mount '{}' (priority {}) for '{}'",
                mount_path,
                mount.priority,
                normalised_path
            );
            ...
            if (mount_path != "/" &&
                ...
                CE_LOG(LogLevel::Info,
                    "[VFS] Mount '{}' is not a valid path boundary match",
                    mount_path
                );
            ...
            CE_LOG(LogLevel::Info,
                "[VFS] Candidate provider '{}' with relative path '{}'",
            ...
            if (!mount.file_provider->FileExists(relative_path)) {
                CE_LOG(LogLevel::Info,
                    "[VFS] File '{}' does not exist in provider '{}'",
                    relative_path,
                    mount.file_provider->GetProviderName()
                );
            ...
            if (!best_mount ||
                ...
                CE_LOG(LogLevel::Info,
                    "[VFS] Selecting provider '{}' at mount '{}'",
                    mount.file_provider->GetProviderName(),
                    mount_path
                );
        ...
        if (!best_mount) {
            CE_LOG(LogLevel::Warn,
                "[VFS] Could not resolve existing file '{}'",
                normalised_path
            );
        ...
        CE_LOG(LogLevel::Info,
            "[VFS] Resolved '{}' to provider '{}' with relative path '{}'",
            normalised_path,
            best_mount->file_provider->GetProviderName(),
            best_relative_path
        );
    ...
    VFS::ResolvedPath VFS::ResolvePathForWrite(std::string_view path) {
        ...
        for (auto& mount : mMountPoints) {
            ...
            CE_LOG(LogLevel::Debug,
                "[VFS] Checking mount '{}' (priority {}) for writable path '{}'",
                mount_path,
                mount.priority,
                normalised_path
            );
            ...
            if (mount.file_provider->IsReadOnly()) {
                CE_LOG(LogLevel::Info,
                    "[VFS] Skipping read-only provider '{}' at mount '{}'",
                    mount.file_provider->GetProviderName(),
                    mount_path
                );
            ...
            CE_LOG(LogLevel::Info,
                "[VFS] Writable candidate provider '{}' with relative path '{}'",
                mount.file_provider->GetProviderName(),
                relative_path
            );
            ...
            if (!best_mount ||
                ...
                (mount_path.length() == best_mount->mount_point.length() &&
                    mount.priority > best_mount->priority)) {
                CE_LOG(LogLevel::Info,
                    "[VFS] Selecting writable provider '{}' at mount '{}'",
                    mount.file_provider->GetProviderName(),
                    mount_path
                );
        ...
        if (!best_mount) {
            CE_LOG(LogLevel::Warn,
                "[VFS] Could not resolve writable provider for '{}'",
                normalised_path
            );
        ...
        CE_LOG(LogLevel::Info,
            "[VFS] Resolved writable path '{}' to provider '{}' with relative path '{}'",
            normalised_path,
            best_mount->file_provider->GetProviderName(),
            best_relative_path
        );
    ...
    bool VFS::FileExists(std::string_view path) {
        CE_LOG(LogLevel::Info,
            "[VFS] Checking whether file '{}' exists",
            path
        );
    ...
    bool VFS::GetFileSize(
        std::string_view path,
        uint64_t& out_size
    ) {
        CE_LOG(LogLevel::Info,
            "[VFS] Getting file size for '{}'",
            path
        );
        ...
        if (!resolved.provider) {
            CE_LOG(LogLevel::Error,
                "[VFS] Cannot get file size: '{}' could not be resolved",
                path
            );
        ...
        if (!resolved.provider->GetFileSize(
            resolved.relative_path,
            out_size
        )) {
            CE_LOG(LogLevel::Error,
                "[VFS] Provider '{}' failed to get file size for '{}'",
                resolved.provider->GetProviderName(),
                resolved.relative_path
            );
    ...
    std::unique_ptr<IFile> VFS::OpenFile(
        ...
        if (!resolved.provider) {
            CE_LOG(LogLevel::Error,
                "[VFS] Cannot open file '{}': no suitable provider found",
                path
            );
        ...
        if (wants_write && resolved.provider->IsReadOnly()) {
            CE_LOG(LogLevel::Error,
                "[VFS] Cannot open '{}' for writing: provider '{}' is read-only",
                path,
                resolved.provider->GetProviderName()
            );
        ...
        CE_LOG(LogLevel::Info,
            "[VFS] Opening '{}' through provider '{}' with relative path '{}'",
            path,
            resolved.provider->GetProviderName(),
            resolved.relative_path
        );

        auto file = resolved.provider->OpenFile(
            resolved.relative_path,
            flags
        );
    ...
    bool VFS::CreateFile(std::string_view path) {
        ...
        if (!resolved.provider) {
            CE_LOG(LogLevel::Error,
                "[VFS] Failed to create file '{}': no writable provider found",
                path
            );
        ...
        if (resolved.provider->IsReadOnly()) {
            CE_LOG(LogLevel::Error,
                "[VFS] Cannot create '{}': provider '{}' is read-only",
                path,
                resolved.provider->GetProviderName()
            );
        ...
        CE_LOG(LogLevel::Info,
            "[VFS] Creating '{}' through provider '{}' with relative path '{}'",
            path,
            resolved.provider->GetProviderName(),
            resolved.relative_path
        );
        ...
        CE_LOG(LogLevel::Info,
            "[VFS] Successfully created file '{}' using provider '{}'",
            path,
            resolved.provider->GetProviderName()
        );
