...
namespace CE::TDF {
    namespace {
        ...
        Value makeSingleElementArray(const Value &val) {
            ...
            case Type::Int32:
                return File::makeIntArray({File::readInt(val)});
            case Type::UInt32:
                return File::makeUIntArray({File::readUInt(val)});
            case Type::Float:
                return File::makeFloatArray({File::readFloat(val)});
            case Type::String:
                return File::makeStringArray({File::readString(val)});
            case Type::Object:
                return File::makeObjectArray({File::readObject(val)}, kObjectVersion);
            default:
                throwFormat("unsupported array element type");
            }
        }
    ...
    Value File::arrayElement(const Value &v, size_t index) {
        ...
        switch (v.type) {
        case Type::ArrBool:
            return makeBool(readBoolArray(v)[index]);
        case Type::ArrInt32:
            return makeInt(readIntArray(v)[index]);
        case Type::ArrUInt32:
            return makeUInt(readUIntArray(v)[index]);
        case Type::ArrFloat:
            return makeFloat(readFloatArray(v)[index]);
        case Type::ArrString:
            return makeString(readStringArray(v)[index]);
        case Type::ArrObject:
            return makeObject(readObjectArray(v)[index], kObjectVersion);
        default:
            throwFormat("unknown array type");
        }
    }
    ...
    size_t File::elementSize(Type arrType, Type elemType) {
        ...
        case Type::Object:
            return 0;
        default:
            throwFormat("unsupported element type");
        }
    }
