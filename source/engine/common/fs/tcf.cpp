...
static void init_tables() {
    ...
    for (int i = 0; i < 256; i++) {
        shift_table[i] = (uint8_t)(((i << SHIFT_BITS) & 0xFF) | (i >> (8 - SHIFT_BITS)));
        unshift_table[i] = (uint8_t)((i >> SHIFT_BITS) | (i << (8 - SHIFT_BITS)));
    }
...
static uint64_t read_u64(std::ifstream& f) {
    ...
    return static_cast<uint64_t>(b[0]) | (static_cast<uint64_t>(b[1]) << 8) | (static_cast<uint64_t>(b[2]) << 16) |
           (static_cast<uint64_t>(b[3]) << 24) | (static_cast<uint64_t>(b[4]) << 32) |
           (static_cast<uint64_t>(b[5]) << 40) | (static_cast<uint64_t>(b[6]) << 48) |
...
int tcf_pack(const char* input_dir, const char* out_path) {
    ...
    // index_offset @ 8
    {
        ...
        for (int i = 0; i < 8; ++i)
            header[8 + i] = static_cast<uint8_t>((v >> (8 * i)) & 0xFF);
    }
    ...
    // file_count @ 16
    {
        ...
        for (int i = 0; i < 8; ++i)
            header[16 + i] = static_cast<uint8_t>((v >> (8 * i)) & 0xFF);
    }
    ...
    // data_offset/header_size @ 24
    {
        ...
        for (int i = 0; i < 8; ++i)
            header[24 + i] = static_cast<uint8_t>((v >> (8 * i)) & 0xFF);
    }
