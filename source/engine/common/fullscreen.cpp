...
namespace CE {
    bool ApplyFullscreenMode(SDL_Window *window, int width, int height) {
        ...
        if (displayID == 0) {
            CE_LOG(CE::LogLevel::Warn, "[Window] Failed to find display for window, using desktop fullscreen: {}",
                   SDL_GetError());
        ...
        const bool foundMode =
            SDL_GetClosestFullscreenDisplayMode(displayID, width, height, refreshRate, false, &fullscreenMode);

        if (!foundMode) {
            CE_LOG(CE::LogLevel::Warn,
                   "[Window] No matching exclusive fullscreen mode for {}x{}, using desktop fullscreen: {}", width,
                   height, SDL_GetError());
            return SDL_SetWindowFullscreenMode(window, nullptr) && SDL_SetWindowFullscreen(window, true);
        }
        ...
        if (!SDL_SetWindowFullscreenMode(window, &fullscreenMode)) {
            CE_LOG(CE::LogLevel::Warn,
                   ...
                   fullscreenMode.w, fullscreenMode.h, fullscreenMode.refresh_rate, SDL_GetError());
            return SDL_SetWindowFullscreenMode(window, nullptr) && SDL_SetWindowFullscreen(window, true);
        }

        if (!SDL_SetWindowFullscreen(window, true)) {
            CE_LOG(CE::LogLevel::Error, "[Window] Failed to enter fullscreen after selecting {}x{} @ {} Hz: {}",
                   fullscreenMode.w, fullscreenMode.h, fullscreenMode.refresh_rate, SDL_GetError());
            return false;
        }

        CE_LOG(CE::LogLevel::Info, "[Window] Fullscreen mode set to {}x{} @ {} Hz", fullscreenMode.w, fullscreenMode.h,
