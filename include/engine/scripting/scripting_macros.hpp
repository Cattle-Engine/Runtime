...
#define CE_REGISTER_NAME_REGISTRY_GLOBAL_WRAPPER(registry, cppType, scriptType, namespaceName, handleMember)           \
    CE_REGISTER_NAME_REGISTRY_GLOBAL_WRAPPER_IMPL(registry, cppType, scriptType, namespaceName, handleMember,          \
                                                  __COUNTER__)
...
#define CE_REGISTER_NAME_REGISTRY_GLOBAL_IMPL(registry, cppType, scriptType, namespaceName, id)                        \
    do {                                                                                                               \
        ...
        mScriptEngine->SetDefaultNamespace(namespaceName);                                                             \
                                                                                                                       \
        if (mScriptEngine->RegisterGlobalFunction("bool Exists(const string &in)",                                     \
                                                  asFUNCTION(CE_CONCAT(_ce_registry_wrapper_, id)::Exists),            \
                                                  asCALL_CDECL) < 0) {                                                 \
            return false;                                                                                              \
        }                                                                                                              \
                                                                                                                       \
        if (mScriptEngine->RegisterGlobalFunction("void Add(const string &in, const " scriptType " &in)",              \
                                                  asFUNCTION(CE_CONCAT(_ce_registry_wrapper_, id)::Add),               \
                                                  asCALL_CDECL) < 0) {                                                 \
            return false;                                                                                              \
        }                                                                                                              \
                                                                                                                       \
        if (mScriptEngine->RegisterGlobalFunction("const " scriptType " &Get(const string &in)",                       \
                                                  asFUNCTION(CE_CONCAT(_ce_registry_wrapper_, id)::Get),               \
                                                  asCALL_CDECL) < 0) {                                                 \
            return false;                                                                                              \
        }                                                                                                              \
                                                                                                                       \
        if (mScriptEngine->RegisterGlobalFunction("void Remove(const string &in)",                                     \
        ...
        if (mScriptEngine->RegisterGlobalFunction(                                                                     \
                "void Clear()", asFUNCTION(CE_CONCAT(_ce_registry_wrapper_, id)::Clear), asCALL_CDECL) < 0) {          \
            return false;                                                                                              \
        }                                                                                                              \
                                                                                                                       \
