...
#define CE_REGISTER_GLOBAL(type, obj, decl, method)                                                                    \
    do {                                                                                                               \
        if (mScriptEngine.RegisterGlobalFunction(decl, asMETHOD(type, method), asCALL_THISCALL_ASGLOBAL, obj) < 0) {   \
            return false;                                                                                              \
        }                                                                                                              \
    } while (false)
...
#define CE_REGISTER_OBJECT_BEHAVIOUR(obj, behaviour, declaration, func_ptr, call_conv)                                 \
    do {                                                                                                               \
        if (mScriptEngine.RegisterObjectBehaviour(obj, behaviour, declaration, func_ptr, call_conv) < 0) {             \
            return false;                                                                                              \
        }                                                                                                              \
    } while (false)
...
#define CE_REGISTER_OBJECT_PROPERTY(obj, decl, byte_offset)                                                            \
    do {                                                                                                               \
        if (mScriptEngine.RegisterObjectProperty(obj, decl, byte_offset) < 0) {                                        \
            return false;                                                                                              \
        }                                                                                                              \
    } while (false)
...
#define CE_REGISTER_OBJECT_METHOD(obj, declaration, func_ptr, call_conv)                                               \
    do {                                                                                                               \
        if (mScriptEngine.RegisterObjectMethod(obj, declaration, func_ptr, call_conv) < 0) {                           \
            return false;                                                                                              \
        }                                                                                                              \
    } while (false)
