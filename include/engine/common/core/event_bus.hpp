...
namespace CE::Core {
    class EventBus {
      public:
        ...
        template <typename Event> int Subscribe(Handler<Event> handler);

        template <typename Event> void Emit(const Event& event);

        template <typename Event> void Unsubscribe(int id);
      ...
      private:
        ...
        void EmitBucket(std::string_view state, std::string_view eventName, std::string_view emittedState,
